...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		$isDB = false;
		if("Course" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Tutor" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Module" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Is_enrolled" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Tutors_a" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Includes" == $this->xml_array['tables'][0])
			$isDB = true;
		if("Student" == $this->xml_array['tables'][0])
			$isDB = true;
		return $isDB;
	}
