...
require_once("include/Module_variables.php");
...
$page_layouts["Module_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	Section - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Section", $data, $keylink);
			$row["Section_value"] = $value;
			$format = $pSet->getViewFormat("Section");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Section")))
				$class = ' rnr-field-number';
			$row["Section_class"] = $class;
	//	cID - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("Module", "detailspreview"));
