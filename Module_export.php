...
$page_layouts["Module_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["Term"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["cID"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["Term"] = urldecode($arr[0]);
			$keys["cID"] = urldecode($arr[1]);
			$selected_recs[] = $keys;
		}
	}
