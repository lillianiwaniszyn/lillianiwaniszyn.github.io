...
require_once("include/Tutor_variables.php");
...
$page_layouts["Tutor_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	//	mInit - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("mInit", $data, $keylink);
			$row["mInit_value"] = $value;
			$format = $pSet->getViewFormat("mInit");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("mInit")))
				$class = ' rnr-field-number';
			$row["mInit_class"] = $class;
	//	lName - 
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("Tutor", "detailspreview"));
